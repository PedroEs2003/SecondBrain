...
const moodExpressions: Record<Mood, {
  mouthType: "smile" | "open" | "small" | "yawn" | "wow";
  eyeScale: number;
  eyeY: number;
  blinkSpeed: number;
  bodyBounce: number;
  headTilt: number;
  accessory?: "umbrella" | "sunglasses" | "zzz" | "sparkles" | "confetti";
}> = {
...
const CartoonMesh = ({ speed = 1, mood = "chill", customization }: { speed?: number; mood?: Mood; customization?: CartoonCustomization }) => {
  ...
  const skinColor = customization?.skinTone || "#ffe0c0";
  const hairColor = customization?.hairColor || "#6b4226";
  const shirtColor = customization?.shirtColor || "#2563eb";
  const eyeColor = customization?.eyeColor || "#3a2a1a";
  ...
  return (
    <group ref={groupRef} scale={0.85} position={[0, -0.1, 0]}>
      ...
      {/* Eyebrows */}
      <mesh position={[-0.17, 0.38, 0.48]} rotation={[0, 0, mood === "worried" ? -0.3 : 0.15]}>
...
const CartoonAvatar = ({ size = 120, className = "", mood = "chill", customization }: { size?: number; className?: string; mood?: Mood; customization?: CartoonCustomization }) => {
  ...
  return (
    <div className={className} style={{ width: size, height: size }}>
      <Canvas camera={{ position: [0, 0, 2.5], fov: 45 }} gl={{ alpha: true, antialias: true }} style={{ background: "transparent" }} dpr={[1, 2]}>
        <Suspense fallback={null}>
          <ambientLight intensity={mood === "sleepy" ? 0.3 : mood === "rain" ? 0.4 : 0.6} />
          <directionalLight position={[3, 5, 5]} intensity={mood === "sleepy" ? 0.3 : 0.7} />
          <directionalLight position={[-2, 2, 3]} intensity={0.3} color={mood === "sleepy" ? "#8888cc" : "#ffe0c0"} />
